...
class _MyAppState extends State<MyApp> {
  ...
  for (var message in _messages) {
    var amount = extractAmountFromMessage(message.body);
    if (amount != null) {
      var category = getCategoryFromMessage(message.body);

      var existingCategory = _categories.firstWhere(
        (cat) => cat.name == category.name,
        orElse: () => Category(name: category.name, amount: 0, icon: category.icon),
      );
...
double? extractAmountFromMessage(String? message) {
  ...
  if (message != null) {
    if (message.contains('slice')) {
      ...
      return Category(name: 'Food Delivery', amount: 0, icon: Icons.delivery_dining);
    } else if (message.contains('EMI') || message.contains('loan')) {
      return Category(name: 'EMI/Loan', amount: 0, icon: Icons.monetization_on);
    }
  }
